...
class _AddDevicePageState extends State<AddDevicePage> {
  ...
  Future<void> _scanAll() async {
    ...
    try {
      ...
      // 3) Escanear
      final results = await BleService.I.scanOleo(timeout: const Duration(seconds: 6));
      if (!mounted) return;
      ...
      if (_scanResults.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No se detectaron dispositivos OLEO cercanos.')),
        );
      } else {
        _nextStep();
      }
  ...
  Widget _step2() => Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          ElevatedButton(
            onPressed: _scanResults.isEmpty || _selectedIndex == null || _wifiLoading
                ? null
                : _fetchWifiNetworks,
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.blueAccent,
